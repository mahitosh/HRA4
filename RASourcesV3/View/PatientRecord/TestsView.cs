...
namespace RiskApps3.View.PatientRecord
{
    public partial class TestsView : HraView
    {
        ...
        private void theBreastImagingHxLoaded(HraListLoadedEventArgs e)
        {
            ...
            if (InitialDx != null)
            {
                if (proband.breastImagingHx.Contains(InitialDx as BreastImagingStudy))
                {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (o != null)
            {
                if (proband.labsHx.Contains(dx as LabResult))
                {
                    proband.labsHx.RemoveFromList(dx as LabResult, SessionManager.Instance.securityContext);
                }
                else if (proband.breastImagingHx.Contains(dx as BreastImagingStudy))
                {
                    proband.breastImagingHx.RemoveFromList(dx as BreastImagingStudy, SessionManager.Instance.securityContext);
                }
                else if (proband.transvaginalImagingHx.Contains(dx as TransvaginalImagingStudy))
                {
                    proband.transvaginalImagingHx.RemoveFromList(dx as TransvaginalImagingStudy, SessionManager.Instance.securityContext);
                }
            }
        }
