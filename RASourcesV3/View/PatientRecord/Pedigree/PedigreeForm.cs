...
using Patient = RiskApps3.Model.PatientRecord.Patient;
...
namespace RiskApps3.View.PatientRecord.Pedigree
{
    public partial class PedigreeForm : HraView
    {
        ...
        public PedigreeForm()
        {
            ...
            SettingsForm.FrameRateChanged+=PedigreeFrameRateChanged;
        ...
        public void PedigreeModelLoaded(PedigreeModel model)
        {
            if (InvokeRequired)
            {
                PedigreeModelLoadedCallback rmc = PedigreeModelLoaded;
            ...
            else
            {
                ...
                float target = ZoomSlider.Value / (float)((ZoomSlider.Maximum - ZoomSlider.Minimum) / 2);
                pedigreeControl1.model.parameters.scale = target;
                pedigreeControl1.model.parameters.verticalSpacing = colorSlider1.Value;

                double hscrollRatio = hScrollBar1.Value / (double)hScrollBar1.Maximum;
                double vscrollRatio = vScrollBar1.Value / (double)vScrollBar1.Maximum;

                pedigreeControl1.model.parameters.hOffset = -1 * ((int)((pedigreeControl1.model.displayXMax * hscrollRatio) - ((double)(Width / 2)) / target));
                pedigreeControl1.model.parameters.vOffset = -1 * ((int)((pedigreeControl1.model.displayYMax * vscrollRatio) - ((double)(Height / 2)) / target));

                hScrollBar1.Minimum = 0;
                hScrollBar1.Maximum = (int)model.displayXMax;
                hScrollBar1.Value = (int)(hScrollBar1.Maximum * hscrollRatio);
                vScrollBar1.Minimum = 0;
                vScrollBar1.Maximum = (int)model.displayYMax;
                vScrollBar1.Value = (int)(vScrollBar1.Maximum * vscrollRatio);
        ...
        private void UpdateComment()
        {
            if (InvokeRequired)
            {
                UpdateCommentCallback apc = UpdateComment;
                Invoke(apc, null);
            }
            else
            {
            ...
            if (e.hraOperand != null)
            {
                ...
                switch (e.hraListChangeType)
                {
                    ...
                    case HraListChangedEventArgs.HraListChangeType.DELETE:
                        ...
                        {
                            FamilialVariants = fhx.ReloadFamilialVariants();
        ...
        private void ProcessGeneticTest()
        {
            ...
            lock (FamilialVariants)
            {
                ...
                label = fhx.BuildFamilialVariantsLabel();
            }
            pedigreeTitleBlock1.SetVariantLabel(label, currentPrefs);
        }
        ...
        /**************************************************************************************************/
        private void CenterPedigreeButton_Click(object sender, EventArgs e)
        {
        ...
        private void toolStripButton2_Click(object sender, EventArgs e)
        {
            ...
            if (SettingsForm.preferences.owningPatient==null)
            {
                ...
                SettingsForm.preferences.SetForm(Text);
        ...
        private GUIPreference getBestFitExistingGuiPreference(bool exactMatchOnly)
        {
            ...
            String parentFormText = (ParentForm != null) ? ParentForm.Text : "";
            ...
            if (localList.Count == 0)
            {
                ...
                guiPreference = new GUIPreference(proband, DateTime.Now, Text, parentFormText, Width, Height);
        ...
        private void ApplyPrefs()
        {
            if (InvokeRequired)
            {
                ApplyPrefsCallback apc = ApplyPrefs;
                Invoke(apc, null);
            }
            else
            {
        ...
        private void PedigreeForm_Resize(object sender, EventArgs e)
        {
            if (pedigreeControl1.model != null)
            {
                CenterPedigreeButton_Click(sender, e);
            ...
            int usableWidth = Width - pedigreeLegend1.Location.X - 40;
            if (usableWidth > 0)
            {
                pedigreeLegend1.CalculateOptimalDimensions(usableWidth);
            }
        }
        ...
        private void toolStripButton16_Click(object sender, EventArgs e)
        {
            if (proband != null)
            {
                Globals.setApptID(proband.apptid);
                Globals.setUnitNum(proband.unitnum);
                frmLinkFamilies frm = new frmLinkFamilies();
                frm.ShowDialog();
                Globals.setApptID(-1);
                Globals.setUnitNum("");
            }
