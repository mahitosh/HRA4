...
namespace RiskApps3
{
    public partial class MainForm : Form
    {
        ...
        {
            AddEditCopyApptController controller = new AddEditCopyApptController(this);
            controller.AddAppt(-1);
        ...
        private void updateApptListFromServerToolStripMenuItem_Click(object sender, EventArgs e)
        {
            InterfaceUtils.updateApptListFromMWL("");  // jdg 1/22/16
        }

        private void utilitiesToolStripMenuItem_Click(object sender, EventArgs e)
        {
            string strUsingMWL = "";
            bool bUsingMWL = false;

            strUsingMWL = RiskApps3.Utilities.Configurator.getNodeValue("Globals", "UsingMWLApptFeed");
            if(!String.IsNullOrEmpty(strUsingMWL))
            {
                bUsingMWL = ((strUsingMWL.ToUpper() == "TRUE" ? true : false));
            }

            if (bUsingMWL)
            {
                updateApptListFromServerToolStripMenuItem.Visible = true;
            }
        }
