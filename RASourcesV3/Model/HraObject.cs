...
namespace RiskApps3.Model
{
    ...
    public abstract class HraObject
    {
        ...
        public bool ReadOnly {
            [PublicAPI] get
            {
                if (this._changeEventArgQueue != null)
                {
        ...
        private static void AuditLogField(string user, int apptId, string unitnum, string type,  string valueList, int relativeId)
        {
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(BCDB2.Instance.getConnectionString()))
                {
                    ...
                    int ret = cmdProcedure.ExecuteNonQuery();
                    connection.Close();
        ...
        public FieldInfo GetHraFieldWithNameLike(string memberName)
        {
            if (this._fields == null)
            {
                this._fields =
                    this.GetType()
                        .GetFields()
                        .Where(field => field.GetCustomAttributes(typeof (HraAttribute), false).Any())
                        .ToArray();
            }
            return this._fields.FirstOrDefault(field => field.Name.Contains(memberName));
        }
