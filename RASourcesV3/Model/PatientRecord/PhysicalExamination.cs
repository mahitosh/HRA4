...
namespace RiskApps3.Model.PatientRecord
{
    ...
    public class PhysicalExamination : HraObject
    {
        ...
        [DataMember]
        [HraAttribute]
        private string lastBreastExamDate;
        [DataMember]
        [HraAttribute]
        ...
        {
            String BMIStr = "";
            if (!string.IsNullOrEmpty(heightInches) && !string.IsNullOrEmpty(weightPounds))
            {
                float heightinchesf = -1;
                float weightPoundsf = -1;
                if (float.TryParse(heightInches, out heightinchesf))
                {
                    if (float.TryParse(weightPounds, out weightPoundsf))
                    {
                        float BMIf = weightPoundsf / (heightinchesf * heightinchesf) * 703;
                        BMIStr = string.Format("{0:F1}", BMIf);
                    }
                }
            }
           PhysicalExamination_BMI = BMIStr;
        }
