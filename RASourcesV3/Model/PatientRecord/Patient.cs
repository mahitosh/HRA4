...
using RiskAppCore;
using RiskApps3.HraRiskAggregator;
...
// jdg 8/31/15 ported from web tablet...
...
namespace RiskApps3.Model.PatientRecord
{
    ...
    {
        //TODO deal with addresses and other demographic information!

        /**************************************************************************************************/
        [DataMember] public string unitnum;
        [DataMember] public int apptid;
        [DataMember] public DateTime apptdatetime;
        [DataMember] [Hra] public string family_comment;
        ...
        public Patient(string pUnitnum)
        {
            unitnum = pUnitnum;
        ...
        public void AddStockRelatives()
        {
            try
            {
                ...
                {
                    connection.Open();
                    SqlCommand command = new SqlCommand("sp_createStockRelatives", connection)
                    {
        ...
        public void RunAutomation()     // jdg 8/31/15 ported from SimpleInfoScreen per Brian
        {
            ...
            {
            ParameterCollection pc = new ParameterCollection("unitnum", unitnum);
            BCDB2.Instance.ExecuteNonQueryWithParams("DELETE FROM tblPrintQueue WHERE unitnum=@unitnum AND printed IS NULL", pc);

            pc = new ParameterCollection("apptid", apptid);
            reader = BCDB2.Instance.ExecuteReaderSPWithParams("sp_AutomationHtmlDocsToPrint", pc);
            ...
            while (reader.Read())
            {
                ...
                {
                    if(printer.ToUpper() == "PENRAD")
                    {
                        InterfaceUtils.send2Penrad(this, templateId, saveLocation, strServiceBinding, toolsPath, apptid);
                        continue;   // go to next automation document
                    }
                    ...
                    if (printer.ToUpper() == "BATCH")
                    {
                        ...
                        BCDB2.Instance.ExecuteNonQueryWithParams(sqlStr, pc2);
                    }
                    else
                    {
                        ...
                        if (printer.ToUpper().StartsWith("SAVE AS"))
                        {
                            ...
                            if (printer.ToUpper() == "SAVE AS PDF")
                            {
                                ...
                                try
                                {
                                    ...
                                    InterfaceUtils.send2PACS(dt, this, fInfo.FullName, apptid);
                                }
                                catch (Exception e)
                                {
                                    Logger.Instance.WriteToLog("MakeDocumentWorker: Error when converting to PDF or bubbled up from send2PACS from appt# " + apptid + ", filename: " + fInfo.FullName + "; underlying error = " + e.Message);
                                }
                            }
                            else
                            {
                        ...
                        else
                        {
                            if (printer.ToUpper() == "POWERSCRIBE")
                            {
                                ...
                                reader = BCDB2.Instance.ExecuteReader(interfaceSql);
                            ...
                            else
                            {
                                ...
                                {
                                    eventLog.Source = "HRA";
                                    eventLog.WriteEntry("apptid: " + apptid.ToString() + " - sending docID " + dt.documentTemplateID.ToString() + " to " + printer, EventLogEntryType.Information, 101, 1);
                                }
            ...
            try
            {
                BCDB2.Instance.ExecuteNonQuery("UPDATE tblRiskData SET printed = 1 WHERE apptid = " + apptid);
            }
            catch (Exception e)
            {
                // If this error occurs, you would never have got here in the first place, seems to me.
                Logger.Instance.WriteToLog("MakeDocumentWorker:  Could not mark appointment #" + apptid + " as printed.  Underlying error = " + e);
            }
        ...
        private DocumentTemplate MakeDocument(Patient proband, int templateID, string templatesRoot)     // ported from SimpleInfoScreen jdg 8/31/15 per Brian
        {
            DocumentTemplate dt = new DocumentTemplate {documentTemplateID = templateID};
        ...
        public void RecalculateRisk()
        {
            ...
            if (Configurator.useAggregatorService())
            {
                ...
                FamilyHistory theFH = this.owningFHx;
                ...
                {
                    toolsPath = Configurator.getNodeValue("Globals", "ToolsPath"); // @"C:\Program Files\riskappsv2\tools\";
                }
                ...
                Service1Client client = new Service1Client(strServiceBinding);
                ...
                try
                {
                    ...
                    if (bmrsInfo != null)
                    {
                        ...
                        DateTime dateValue;
                        if (DateTime.TryParseExact(hl7effTime, formats, null, DateTimeStyles.None, out dateValue))
                    ...
                    //RP.BackgroundLoadWork();    // jdg 11/18/15 commented out after causing problem.... 12/3/15
                }
            ...
            RP.riskCode = fetchPenradValue(apptid, 1000);
            RP.riskText = fetchPenradValue(apptid, 1010);
            RP.riskFormattedText = fetchPenradValue(apptid, 1020);
        ...
        /// <summary>
        /// gets paragraph text based on rule... not really specific to Penrad... but so it begins
        /// </summary>
        /// <param name="apptid"></param>
        /// <param name="rule"></param>
        /// <returns>paragraph text as specified by apptid and rule number</returns>
        private string fetchPenradValue(int apptid, int rule)
        {
        ...
        private void processBrcaProHL7Scores(XPathNavigator nav)
        {
            ...
            if (bpNode != null)
            {
                ...
                myTempDoc.LoadXml("<text>" + HttpUtility.HtmlDecode(textNode.InnerXml) + "</text>");
        ...
        private void processTyrerCuzickHL7Scores(XPathNavigator nav)
        {
            ...
            if (tcNode != null)
            {
                ...
                //create XML fragment with messages and hazard rates
                XmlDocument myTempDoc = new XmlDocument();
                myTempDoc.LoadXml("<text>" + (textNode != null ? HttpUtility.HtmlDecode(textNode.InnerXml) : "<messages>No TC Messages</messages>") + "</text>");
                XPathNavigator n1 = myTempDoc.CreateNavigator();
                ...
                foreach (XPathNavigator cpr in ptCPRIter)
                {
                    switch (cpr.Evaluate("string(code/@displayName)").ToString())
                    {
                        ...
                        case "probBRCA1Mutation":
                            ...
                            RP.TyrerCuzick_Brca_1 = Convert.ToDouble(RP.TyrerCuzickModel.RiskFactors.TYRER_CUZICK_1);   // jdg 11/17/15
                            break;
                        case "probBRCA2Mutation":
                            RP.TyrerCuzickModel.RiskFactors.TYRER_CUZICK_2 = (100.0 * Convert.ToDouble(cpr.Evaluate("string(value/@value)").ToString())).ToString();
                            RP.TyrerCuzick_Brca_2 = Convert.ToDouble(RP.TyrerCuzickModel.RiskFactors.TYRER_CUZICK_2);   // jdg 11/17/15
                            break;
                        default:
                            //do nothing
                            break;
                ...
                foreach (XPathNavigator ac in ptAgeChunkIter)
                {
                    ...
                    if (tcrba == null)
                    {
                        ...
                        if (ptAge >= 0)
                        {
                            ...
                            tcrba.description = (deltaAge % 5 == 0) ? deltaAge + " Year" : "";
                        }
                        HraModelChangedEventArgs args = new HraModelChangedEventArgs(null);
                    ...
                    switch (ac.Evaluate("string(pertinentInformation/realmCode/@code)").ToString())
                    {
                        ...
                        case "populationBreastCancerRisk":
                            ...
                            if (age == five_year_age) RP.TyrerCuzick_5Year_populationRisk = tcrba.PopulationCaRisk;     // jdg 11/17/15
                            if (age == lifetime_age) RP.TyrerCuzick_Lifetime_populationRisk = tcrba.PopulationCaRisk;   // jdg 11/17/15
                            break;
                        default:
                            //do nothing
                            break;
                ...
                foreach (XPathNavigator cpr in ptCPRIter)
                {
                    switch (cpr.Evaluate("string(code/@displayName)").ToString())
                    {
                        ...
                        case "probBRCA1Mutation":
                            ...
                            RP.TyrerCuzick_v7_Brca_1 = Convert.ToDouble(RP.TyrerCuzickModel_v7.RiskFactors.TYRER_CUZICK7_1);   // jdg 11/17/15
                            break;
                        case "probBRCA2Mutation":
                            ...
                            RP.TyrerCuzick_v7_Brca_2 = Convert.ToDouble(RP.TyrerCuzickModel_v7.RiskFactors.TYRER_CUZICK7_2);   // jdg 11/17/15
                            break;
                        default:
                            //do nothing
                            break;
                ...
                foreach (XPathNavigator ac in ptAgeChunkIter)
                {
                    ...
                    switch (ac.Evaluate("string(pertinentInformation/realmCode/@code)").ToString())
                    {
                        ...
                        case "populationBreastCancerRisk":
                            ...
                            if (age == five_year_age) RP.TyrerCuzick_v7_5Year_populationRisk = tcrba.PopulationCaRisk;     // jdg 11/17/15
                            if (age == lifetime_age) RP.TyrerCuzick_v7_Lifetime_populationRisk = tcrba.PopulationCaRisk;   // jdg 11/17/15
                            break;
                        default:
                            //do nothing
                            break;
        ...
        { 
            ...
            if (string.IsNullOrEmpty(targetPerson.firstName))
            {
                targetPerson.firstName = dataRootNode.Evaluate("string(name/given)").ToString();
            }
            if (string.IsNullOrEmpty(targetPerson.lastName))
            {
                targetPerson.lastName = dataRootNode.Evaluate("string(name/family)").ToString();
            }
            if (string.IsNullOrEmpty(targetPerson.name))
            {
            ...
            targetPerson.dateOfDeath = HL7FormatTranslator.DateFromHL7(dataRootNode.Evaluate("string(deceasedTime/@value)").ToString());
            targetPerson.vitalStatus = HL7FormatTranslator.VitalStatusFromHL7(dataRootNode.Evaluate("string(deceasedInd/@code)").ToString());
            ...
            //2015-12-11 PRB excluded PedigreePosition 'diseases'
            XPathNodeIterator coIter = personNode.Select(@"subjectOf2/clinicalObservation[not(sourceOf/@typeCode='CAUS') and not(code/@code='PedigreePosition')]");
        ...
        private void processPREMMScores(XPathNavigator nav)
        {
            ...
            if (premmNode != null)
            {
                ...
                //create XML fragment with messages
                XmlDocument myTempDoc = new XmlDocument();
                myTempDoc.LoadXml("<text>" + (textNode != null ? HttpUtility.HtmlDecode(textNode.InnerXml) : "<messages>No PREMM Messages</messages>") + "</text>");
                XPathNavigator n1 = myTempDoc.CreateNavigator();
