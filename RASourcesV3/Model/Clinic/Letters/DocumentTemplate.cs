...
namespace RiskApps3.Model.Clinic
{
    ...
    /**********************************************************************/
    public class DocumentTemplateList : HRAList<DocumentTemplate>
    {
    ...
    public class DocumentTemplate : HraObject
    {
        ...
        public FileInfo CalculateFileName(string patientName, string date, int apptid, string unitnum, string extension, string directory)
        {
            ...
            // begin jdg 12/2/15 Middlesex is sending first/space/last in their appt feed.  So to accomodate them and any future ones like that, jigger the name parts accordingly.
            // don't split on "space" because a name might legitimately have a space in it, e.g., mary ann smith. 
            if (nameParts.Length == 1)
            {
                int lastIndex = patientName.LastIndexOf(" ");
                if (lastIndex > 0)
                {
                    List<string> parts = new List<string>();
                    parts.Add(patientName.Substring(lastIndex));
                    parts.Add(patientName.Substring(0, lastIndex));
